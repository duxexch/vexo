...
- [Realtime Monitoring and CI Artifacts](#realtime-monitoring-and-ci-artifacts)
...
## Realtime Monitoring and CI Artifacts

### Health KPI Endpoints

Use the KPI endpoint for quick monitoring snapshots (read-only):

- Public KPI: `GET /api/health/kpi`
- Admin KPI alias (requires admin bearer token): `GET /api/admin/health/kpi`

Example payload fields include:

- `status` (`healthy`, `degraded`, `unhealthy`)
- `kpi.databaseLatencyMs`
- `kpi.redisConnected`
- `kpi.minioConnected`
- `kpi.connectedUsers`
- `kpi.totalWsConnections`
- `kpi.activeVoiceRooms`
- `kpi.voiceJoinAcceptanceRate`

### Voice Signaling CI Artifact

Run the CI command to execute the voice signaling load smoke and emit a structured JSON artifact:

```bash
npm run quality:perf:voice-signaling:ci
```

Artifact output path:

- `output/ci-artifacts/voice-signaling-load.json`

Pipeline notes:

- The command exits with non-zero status when any scenario round fails.
- The artifact is written after startup and scenario execution begin; early input/setup failures (for example invalid scenarios JSON path/format) can fail before artifact creation.
