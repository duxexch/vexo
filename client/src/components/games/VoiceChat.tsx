...
export function VoiceChat({
  ...
  const ensureLocalStream = useCallback(async (): Promise<MediaStream | null> => {
    ...
    // `granted`, so we MUST resolve the plugin call before invoking
    // `getUserMedia` — otherwise Capacitor 8's BridgeWebChromeClient
    // auto-rejects the WebView's mic request and the user sees the
