...
class NativeCallUIPlugin : Plugin() {
    ...
    override fun load() {
        instance = this
        // NOTE: We deliberately DO NOT swap the WebView's
        // `WebChromeClient` here. Capacitor's `BridgeWebChromeClient`
        // already implements `onPermissionRequest` correctly — it maps
        // `RESOURCE_AUDIO_CAPTURE` / `RESOURCE_VIDEO_CAPTURE` to the
        // matching Android runtime permissions and routes the request
        // through `ActivityCompat.requestPermissions`, so the OS
        // dialog appears as expected.
        //
        // The root-cause fix lives in the JS layer (`VoiceChat.tsx`,
        // `use-call-session.tsx`, `private-call-layer.tsx`): they call
        // `requestCallMediaPermissions` BEFORE invoking `getUserMedia`,
        // which guarantees the host runtime permission is held by the
        // time the WebView issues its request.
        //
        // Wrapping the WebChromeClient at runtime is brittle (no
        // public getter on `WebView`, reflection breakage risk on
        // future Android versions, and any minimal wrapper would lose
        // file chooser / JS dialog forwarding). Host integrators MUST
        // therefore preserve the bridge's default WebChromeClient — see
        // `examples/AndroidManifest-snippet.xml` for the contract.
    }
