...
export function registerCreateRoute(app: Express) {
  app.post("/api/challenges", sensitiveRateLimiter, authMiddleware, async (req: AuthRequest, res: Response) => {
    try {
      const { gameType, betAmount, opponentType, friendAccountId, visibility = 'public', currencyType = 'usd', requiredPlayers = 2 } = req.body;
      ...
      if (currencyType === 'usd' && parsedBetAmount < 0.01) {
