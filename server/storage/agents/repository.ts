...
export async function getAgentStats(agentId: string): Promise<AgentStatsResponse> {
  ...
  async function aggregate(since: Date): Promise<AgentStatsPeriod> {
    const ledger = await db
      .select({
        type: agentWalletTransactions.type,
        amount: agentWalletTransactions.amount,
      })
...
export async function setAgentActive(agentId: string, isActive: boolean, reason?: string): Promise<Agent | null> {
...
export async function updateAgent(agentId: string, fields: AgentUpdatableFields): Promise<Agent | null> {
