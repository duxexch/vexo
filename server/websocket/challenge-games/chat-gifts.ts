...
import { db } from "../../db";
import { users, challenges, transactions, gameplaySettings, projectCurrencyWallets, projectCurrencyLedger } from "@shared/schema";
import { eq, and, sql } from "drizzle-orm";
import { storage } from "../../storage";
import type { AuthenticatedSocket } from "../shared";
import { requireChallengeParticipant } from "./guards";

// NOTE: Task #9 — challenge chat has been fully migrated to the Socket.IO
// `/chat` namespace. The legacy `handleChallengeChat` handler and its
// `challenge_chat` message type were removed; see `server/socketio/challenge-chat-bridge.ts`.
