...
- [`.github/workflows/android-build.yml`](.github/workflows/android-build.yml) — primary manual build with target `apk | aab | both`. Decodes the keystore from `ANDROID_KEYSTORE_BASE64` (using the safe `printf '%s' | base64 -d` form), patches `android/app/build.gradle` via [`scripts/ci/patch-android-signing.mjs`](scripts/ci/patch-android-signing.mjs), runs gradle `assembleRelease` / `bundleRelease`, copies the outputs to `client/public/downloads/app.apk` and `client/public/downloads/app.aab` (Git LFS-tracked), commits them with `[skip ci]`, and publishes a per-run `android-build-<run>` GitHub Release. Java 17, Node 22.
- [`.github/workflows/release-android.yml`](.github/workflows/release-android.yml) — secondary publish-only flow that verifies the keystore SHA-256 matches the expected fingerprint above (aborts on mismatch), then creates a dated `android-v<UTC-date>-<sha>` GitHub Release and prunes older `android-v…` releases so only the latest two ever exist. Java 17, Node 22.
