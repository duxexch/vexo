...
- 2026-04-24 — **Game Speed picker now scales chess animations too (Task #50):**
  - Renamed `client/src/lib/domino-speed.ts` → `client/src/lib/game-speed.ts` and renamed every export (`DominoSpeedMode` → `GameSpeedMode`, `useDominoSpeedMultiplier` → `useGameSpeedMultiplier`, etc.). Kept the `localStorage` key as `vex.dominoSpeedMode` so existing user prefs survive the rename. Updated all three call sites: `client/src/pages/settings.tsx`, `client/src/hooks/useGameWebSocket.ts`, `client/src/components/games/DominoBoard.tsx`.
  - Both chess board files now consume `useGameSpeedMultiplier`: the 1022-line `client/src/components/games/ChessBoard.tsx` (used by `challenge-game.tsx` + `challenge-watch.tsx`) and the 451-line themed `client/src/components/games/chess/ChessBoard.tsx` (used by `/games/chess`). The themed board scales its inline transition durations directly. The 1022-line board overrides `style.animationDuration` inline for the three CSS animations defined in `client/src/index.css` (`animate-chess-move`, `animate-chess-capture`, `animate-chess-capture-ring`) — so Normal/Fast/Turbo actually shorten the animation itself rather than just trimming the cleanup timer. Reduced-motion (`speedMultiplier === 0`) now skips both the slide animation and the capture FX entirely instead of flashing them for ~0ms.
  - `server/lib/adaptive-ai.ts` — added a comment at the trivial-move (`validMoves.length <= 1`) shortcut documenting that the 120–320ms snappy floor is the single source of truth for *every* game type (no behavior change). The server-side per-player speed multiplier in `server/game-websocket/speed-mode.ts` already applies across game types.
  - `client/src/locales/en.ts` + `client/src/locales/ar.ts` — `settings.gameSpeedDescription` updated from "domino animations" to "every classic game (chess, dominoes, and more)" / Arabic equivalent. Translation keys unchanged.
  - Note: original task spec listed `DraughtsBoard.tsx`, but no draughts game exists in this codebase (the 6 games are chess, backgammon, dominoes, tarneeb, baloot, language duel). Implemented for chess only; backgammon / baloot / tarneeb extension proposed as a follow-up.
