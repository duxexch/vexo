...
<<<<<<< HEAD
**Automated CI release pipeline:** A GitHub Actions workflow at [`.github/workflows/release-android.yml`](.github/workflows/release-android.yml) builds, signs, and publishes a fresh APK + AAB on every push to `main` (and on demand via *Run workflow*). It pulls the keystore from the `ANDROID_KEYSTORE_BASE64` repo secret, verifies its SHA-256 matches the expected fingerprint above (build aborts on mismatch), patches `android/app/build.gradle` with the canonical signing block via [`scripts/ci/patch-android-signing.mjs`](scripts/ci/patch-android-signing.mjs), runs `assembleRelease` + `bundleRelease`, re-verifies the APK signature, then publishes a `android-v<UTC-date>-<sha>` GitHub Release containing both binaries — and deletes any older `android-v…` releases so only the latest two ever exist. Setup procedure (one-time secrets, password rotation, deploy-back-to-Hostinger): [`docs/mobile/github-actions-release-setup.md`](docs/mobile/github-actions-release-setup.md).

=======
>>>>>>> 368b268 (تحديث تلقائي للنسخ الموقعة APK/AAB)
